 ...
 * @returns
 */
const createRandomFileName = (file: any) => {
...
export const extractFilepath = (filename: string) => {
  ...
  const dir: string = uploadDirectories[extname(filename).split('.')[1]]
    ? uploadDirectories[extname(filename).split('.')[1]]
    : uploadDirectories['other'];

  return (
    configs.apiurl + '/uploads' + dir.split('/uploads')[1] + '/' + filename
  );
};
...
export const uploader = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      console.log("lter Coming")
      Object.keys(uploadDirectories).forEach((dir) => {
        !existsSync(uploadDirectories[dir]) &&
          mkdirSync(uploadDirectories[dir], { recursive: true });
      });

      cb(
        null,
        uploadDirectories[extname(file.originalname).split('.')[1]]
          ? uploadDirectories[extname(file.originalname).split('.')[1]]
          : uploadDirectories['other'],
      );
    },
